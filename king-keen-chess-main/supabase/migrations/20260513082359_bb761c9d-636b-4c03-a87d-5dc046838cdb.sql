...
-- Tutorial FENs use far-corner kings so only the relevant pieces stand out.
...
(1,1,'tutorial','The Rook Walks','Move the rook to a5','Rooks slide along files!','7k/8/8/8/8/8/8/R6K w - - 0 1','a1a5',1,1),
(1,2,'tutorial','Rook Capture','Capture the pawn on a6','Take the pawn!','7k/8/p7/8/8/8/8/R6K w - - 0 1','a1a6',1,1),
(1,3,'tutorial','The Knight Jumps','Land the knight on f5','Knights jump in an L!','7k/8/8/8/3N4/8/8/7K w - - 0 1','d4f5',1,1),
(1,4,'tutorial','Knight in 2','Reach b3 in 2 moves','Plan ahead!','7k/8/8/8/8/8/8/N6K w - - 0 1','a1c2;c2b3',1,2),
(1,5,'tutorial','The Bishop''s Diagonal','Move the bishop to g7','Bishops love diagonals!','7k/8/8/8/3B4/8/8/7K w - - 0 1','d4g7',1,1),
(1,6,'tutorial','Pawn Push','Move the pawn to e4','Two squares from start!','7k/8/8/8/8/8/4P3/7K w - - 0 1','e2e4',1,1),
(1,7,'tutorial','Pawn Capture','Capture the knight with your pawn','Pawns capture diagonally!','7k/8/8/4n3/3P4/8/8/7K w - - 0 1','d4e5',1,1),
(1,8,'tutorial','Mini Checkmate','Checkmate the king in 1','Corner the King!','4k3/8/8/8/8/8/8/R3K3 w - - 0 1','a1a8',1,1);
...
(3,3,'checkmate','Mate in 1','Find the combination','Force it','6k1/5ppp/8/8/8/8/5PPP/R5K1 w - - 0 1','a1a8',3,1),
...
CREATE POLICY "mq2_select_auth" ON public.minigame_queue FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "mq2_insert_self" ON public.minigame_queue FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "mq2_update_self" ON public.minigame_queue FOR UPDATE USING (auth.uid() = user_id OR auth.uid() = opponent_id);
CREATE POLICY "mq2_delete_self" ON public.minigame_queue FOR DELETE USING (auth.uid() = user_id);
